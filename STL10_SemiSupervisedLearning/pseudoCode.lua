...
model = torch.load('logs/sample_model_with_data_augmentation_final/model.net')

print '==> loading extradata data'
--load extradata
extraData = torch.load('extraData70K.t7').extraData
...
function predict()
  ...
  for i=1,extraData.data:size(1),bs do
    -- i indicate current index
        ...
        local pred = model:forward(extraData.data:narrow(1,i,bs):cuda())
           --tData.data:narrow(1,i,bs):cuda()
        -- get the peudo_label: t, label, score
        for e =1 ,bs do  -- each instance
          local count =0
          score = pred[e]:max()
          --print("pred :",pred[e]," max :",score)
          for it=1, 10 do    
...
torch.save('label70K_aug_Ix.t7',result)
